...
class SaleOrderLine(models.Model):
    ...
    @api.model
    def _skip_pricelist_team_user_sale_order_line_check(self):
        return bool(
            self.env.su
    ...
    @api.model
    def _get_sale_order_line_pricelist_access_domain(self):
    ...
    def _get_team_user_access_domain(self):
        if self._skip_pricelist_team_user_sale_order_line_check():
            return []
        accessible_ids = self.env['product.pricelist']._get_accessible_pricelist_ids(
            admin_bypass=True,
        )
        ...
        return [
            '|',
            ('order_id.pricelist_id', '=', False),
            ('order_id.pricelist_id', 'in', accessible_ids),
        ]
    ...
    def _check_sale_order_line_pricelist_team_user_access(self, operation):
        if self._skip_pricelist_team_user_sale_order_line_check():
            return
        ...
        access_domain = self._get_sale_order_line_pricelist_access_domain()
        ...
        for line in self:
            ...
            if not allowed:
                raise AccessError(_(
                    "You are not allowed to %(operation)s sale order line %(line)s because "
                    "the selected pricelist %(pricelist)s on sale order %(order)s is restricted "
                    "to another sales team/user."
                ) % {
                    'operation': operation,
    ...
    def create(self, vals_list):
        ...
        records._check_sale_order_line_pricelist_team_user_access(_('create'))
        return records

    def write(self, vals):
        self._check_sale_order_line_pricelist_team_user_access(_('edit'))
        res = super().write(vals)
        self._check_sale_order_line_pricelist_team_user_access(_('edit'))
        return res

    def unlink(self):
        self._check_sale_order_line_pricelist_team_user_access(_('delete'))
        return super().unlink()
