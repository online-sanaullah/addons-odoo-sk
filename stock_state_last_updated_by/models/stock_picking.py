...
class StockPicking(models.Model):
    ...
    def _compute_state_last_tracking_info(self):
        ...
        values_by_id = helper._get_latest_state_tracking_values("stock.picking", self.ids)
        for picking in self:
            vals = values_by_id.get(picking.id)
            picking.state_last_updated_by_id = vals.get("user_id") if vals else False
            picking.state_last_updated_on = vals.get("date") if vals else False
            picking.state_last_updated_source = vals.get("source") if vals else False
